...
class UpdatePushNotiData {
    ...
    func pushPostNoti(targetPostID: String, receiveUser: User, type: PostNotiType, message: String, post: Post, sendUser: User) async {
        switch type {
        case .comment:
            ...
            if receiveUser.nameID != userNameID && receiveUser.alertAccept == "yes" {
                if sendUser.username.isEmpty {
                    PushNotificationManager.shared.sendPushNotificationWithPost(
                        toFCMToken: receiveUser.fcmToken,
                        title: "PADO",
                        body: "\(userNameID)(\(sendUser.username))님이 회원님의 파도에 댓글을 남겼습니다: \"\(message)\"",
            ...
            if receiveUser.nameID != userNameID && receiveUser.alertAccept == "yes" {
                if sendUser.username.isEmpty {
                    PushNotificationManager.shared.sendPushNotificationWithPost(
                        toFCMToken: receiveUser.fcmToken,
                        title: "PADO",
                        body: "\(userNameID)(\(sendUser.username))님이 회원님의 파도에 포토모지를 남겼습니다",
                        categoryIdentifier: "post",
                        post: post
                    )
                } else {
            ...
            if receiveUser.nameID != userNameID && receiveUser.alertAccept == "yes" {
                if sendUser.username.isEmpty {
                    PushNotificationManager.shared.sendPushNotificationWithPost(
                        toFCMToken: receiveUser.fcmToken,
                        title: "PADO",
                        body: "\(userNameID)(\(sendUser.username))님이 회원님의 파도에 ❤️로 공감했습니다",
                        categoryIdentifier: "post",
                        post: post
                    )
                } else {
            ...
            if receiveUser.alertAccept == "yes" {
                if sendUser.username.isEmpty {
                    PushNotificationManager.shared.sendPushNotificationWithPost(
                        toFCMToken: receiveUser.fcmToken,
                        title: "PADO",
                        body: "\(userNameID)(\(sendUser.username))님이 회원님에게 파도를 보냈습니다",
                        categoryIdentifier: "post",
                        post: post
                    )
                } else {
            ...
            if receiveUser.alertAccept == "yes" {
                if sendUser.username.isEmpty {
                    PushNotificationManager.shared.sendPushNotificationWithPost(
                        toFCMToken: receiveUser.fcmToken,
                        title: "PADO",
                        body: "\(userNameID)(\(sendUser.username))님이 회원님을 파도탔습니다",
                        categoryIdentifier: "post",
                        post: post
                    )
                } else {
            ...
            if receiveUser.alertAccept == "yes" {
                if sendUser.username.isEmpty {
                    PushNotificationManager.shared.sendPushNotification(
                        toFCMToken: receiveUser.fcmToken,
                        title: "PADO",
                        body: "\(userNameID)(\(sendUser.username))님이 회원님을 팔로우 하기 시작했습니다",
            ...
            if receiveUser.alertAccept == "yes" {
                if sendUser.username.isEmpty {
                    PushNotificationManager.shared.sendPushNotification(
                        toFCMToken: receiveUser.fcmToken,
                        title: "PADO",
                        body: "\(userNameID)(\(sendUser.username))님이 회원님을 서퍼🏄🏼‍♀️로 지정했습니다",
                        categoryIdentifier: "profile",
                        user: sendUser
                    )
                } else {
            ...
            if receiveUser.nameID != userNameID && receiveUser.alertAccept == "yes" {
                if sendUser.username.isEmpty {
                    PushNotificationManager.shared.sendPostitPushNotification(
                        toFCMToken: receiveUser.fcmToken,
                        title: "PADO",
                        body: "\(userNameID)(\(sendUser.username))님이 회원님의 방명록에 글을 남겼습니다: \(message)",
